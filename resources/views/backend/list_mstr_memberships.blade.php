        ...
        <div class="col">
            <div class="card">
                <div class="card-header">
                    <strong>Memberships List</strong>
    ...
    <div class="modal fade" id="create_desig" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true" style="display: none;">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <form method="post" action="{{ route('admin.mstr.membership.store') }}" id="add_desg">
                    {{csrf_field()}}
...
$(function() {
    $('#userTable').DataTable({
        processing: true,
        serverSide: true,
        ajax: "{{ route('admin.mstr.membership.index') }}",
        columns: [
    ...
    $("body").on("click", ".editRow", function(e)
    {
        $.ajax({
            ...
            {
                $("#edit_desg").attr('action',"{{ route('admin.mstr.membership.index') }}/"+result['data']['id']);
    ...
    $("#edit_desg").validate({
        ...
        rules: {
            memb_name : {
                required : true,
                remote: {
                    url: "{{ route('admin.mstr.membership.checkDuplicate') }}",
                    type: "post",
                    data : {
        ...
        messages : {
            memb_name : {
                remote : 'The membership is already exists'
            }
        },
        errorElement: "span",
