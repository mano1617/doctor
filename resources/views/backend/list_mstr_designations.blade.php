    ...
    <div class="modal fade" id="create_desig" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true" style="display: none;">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <form method="post" action="{{ route('admin.mstr.designation.store') }}" id="add_desg">
                    {{csrf_field()}}
                        ...
                        <div class="row">
                            <div class="col-sm-12">
                                <div class="form-group">
                                    ...
                                    <input type="text" id="add_desig_name" name="desig_name" placeholder="Enter designation name" class="form-control">
                    ...
                    <div class="modal-body">
                        <div class="row">
                            <div class="col-sm-12">
                                <div class="form-group">
                                    <label for="name">Name</label>
                                    <input type="text" name="desig_name" id="desig_name" placeholder="Enter designation name" class="form-control">
                                </div>
                            </div>
                        </div>
                    </div>
...
$(function() {
    $('#userTable').DataTable({
        processing: true,
        serverSide: true,
        ajax: "{{ route('admin.mstr.designation.index') }}",
        columns: [
                { data: 'id', name: 'id' },
                { data: 'name', name: 'name' },
    ...
    $("body").on("click", ".editRow", function(e)
    {
        $.ajax({
            ...
            {
                $("#edit_desg").attr('action',"{{ route('admin.mstr.designation.index') }}/"+result['data']['id']);
    ...
    $("#edit_desg").validate({
        ...
        rules: {
            desig_name : {
                required : true,
                remote: {
                    url: "{{ route('admin.mstr.designation.checkDuplicate') }}",
                    type: "post",
                    data : {
        ...
        messages : {
            desig_name : {
                remote : 'The designation is already exists'
            }
        },
        errorElement: "span",
