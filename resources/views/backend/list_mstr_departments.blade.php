    ...
    <div class="modal fade" id="create_desig" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true" style="display: none;">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <form method="post" action="{{ route('admin.mstr.department.store') }}" id="add_desg">
                    {{csrf_field()}}
                        ...
                        <div class="row">
                            <div class="col-sm-4">
                                <div class="form-group">
                                    ...
                                    <select name="degree_type" id="add_degree_type" class="form-control">
                            ...
                            <div class="col-sm-8">
                                <div class="form-group">
                                    ...
                                    <input type="text" id="add_desig_name" name="desig_name" placeholder="Enter department name" class="form-control">
                    ...
                    <div class="modal-body">
                        <div class="row">
                            <div class="col-sm-4">
                                <div class="form-group">
                                    <label for="name">Degree Type</label>
                                    <select name="degree_type" id="edit_degree_type" class="form-control">
                                    <option value="UG">UG</option>
                                    <option value="PG">PG</option>
                                    </select>
                                </div>
                            ...
                            <div class="col-sm-8">
                                <div class="form-group">
                                    <label for="name">Name</label>
                                    <input type="text" name="desig_name" id="desig_name" placeholder="Enter department name" class="form-control">
                                </div>
                            </div>
                        </div>
                    </div>
...
$(function() {
    $('#userTable').DataTable({
        processing: true,
        serverSide: true,
        ajax: "{{ route('admin.mstr.department.index') }}",
        columns: [
    ...
    $("body").on("click", ".editRow", function(e)
    {
        $.ajax({
            ...
            {
                $("#edit_desg").attr('action',"{{ route('admin.mstr.department.index') }}/"+result['data']['id']);
    ...
    $("#edit_desg").validate({
        ...
        rules: {
            desig_name : {
                required : true,
                remote: {
                    url: "{{ route('admin.mstr.department.checkDuplicate') }}",
                    type: "post",
                    data : {
                        '_token' : function() { return '{{ csrf_token() }}' },
                        'rowId' : function() { return $.trim($("input[name='desig_id']").val()) },
                        'degree_type' : function() { return $("#edit_degree_type").val() }
                    }
                }
            }
        },
        messages : {
            desig_name : {
                remote : 'The department is already exists'
            }
        },
        errorElement: "span",
        ...
        rules: {
            desig_name : {
                ...
                remote: {
                    ...
                    data : {
                        '_token' : function() { return '{{ csrf_token() }}' },
                        'degree_type' : function() { return $("#add_degree_type").val(); }
                    }
                }
            }
        },
